...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除、Design doc と PR 説明文の結論先出しを定める。日本語で技術書の章、草稿、記事、解説文、Design doc、PR 説明文、実装計画を書くとき、または推敲・リライトするときに使用する。
---
...
Design doc、PR 説明文、実装計画のように、レビュアーが意思決定を追うための作業ドキュメントには、原稿の規範に加えて次を適用する。

- 冒頭に結論（採用した設計とその理由）を3行以内で書き、詳細はその後に続ける。「論証は一方向に進める」（結論を先に置かない）は原稿の規範であり、作業ドキュメントではこちらを優先する。
- 検討した代替案は「案」と「却下理由」の2列の表にまとめる。案ごとにメリットとデメリットを文章で書き下さない。
- やらないこと（Non-goals）を書く。スコープ外の一般論や将来の拡張可能性を並べない。
- 用語定義、背景、目的、対象読者のような定型の見出しは、読み手がその用語を知らないと判断できない場合だけ書く。テンプレートの見出しを埋めるために中身の薄い節を作らない。
